...
userList.forEach((user) => {
  console.log(`제 이름은 ${user.userName}입니다. ${user.address}에 삽니다`);
});
...
const newUserList = userList.filter((ac) => ac.account === "abc1234");
...
const firstHobby = userList.map((user) => user.hobbys[0]);
console.log(firstHobby);

// filter -> 10개짜리 배열에서 조건에 맞는 몇개만 필터링 한다
// 필터의 결과 배열은 갯수가 10개 이하일 가능성이 크다.

// map -> 10개짜리 배열에서 조건대로 10개를 전부 매핑.
// 맵의 결과 배열 갯수는 무조건 10개다.

// 회원 목록(4명)에서 회원의 이름과 주소만 추출하고 싶음.
const prettierUserList = userList.map((user) => {
  return {
...
appleBasket
  ...
  console.log("=========================");
// 회원 목록에서 서울에 사는 회원들의 2번째 취미만 배열에 모아서
