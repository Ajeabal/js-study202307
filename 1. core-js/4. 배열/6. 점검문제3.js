...
while (true) {
    let inputName = prompt(`${members}\n수정할 멤버 이름을 입력하세요`);
    let isPresent = members.includes(inputName);
    if (isPresent === false) {
      alert(`${inputName}는 잘못된 이름입니다.`);
      continue;
    }
    let inputRename = prompt(`새로운 맴버 이름을 입력하세요`);
    let index = members.indexOf(inputName);
    members.splice(index, 1, inputRename);
    break;
}
alert(`[${members}] 수정된 맴버입니다.`)